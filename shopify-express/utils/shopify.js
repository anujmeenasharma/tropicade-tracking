...
async function getFulfillmentOrderIds(orderId, storeDomain, accessToken) {
  ...
  console.log(`[Express Shopify API] Fetching fulfillment orders for order ${orderId} from https://${cleanDomain}`);
  ...
  if (activeOrders.length === 0) {
    console.warn(`[Express Shopify API] No active/unfulfilled fulfillment orders found for Shopify order ID: ${orderId}`);
    return [];
  }
 ...
 * @param {string} params.trackingNumber - Unique generated tracking ID (e.g. TX-XXXXXX)
 * @param {string} params.trackingUrl - Tracking tracking details URL
 * @param {string} params.carrierName - Carrier name (default: 'Titan X Logistics')
 * @returns {Promise<object>} Result indicator and API response.
 */
async function createShopifyFulfillment({
  ...
  try {
    ...
    if (fulfillmentOrderIds.length === 0) {
      console.warn(`[Express Shopify API] Order ${orderId} cannot be fulfilled (might already be fulfilled or cancelled).`);
    ...
    console.log(`[Express Shopify API] Submitting fulfillment request for Shopify order ${orderId}...`);
    ...
    const data = await response.json();
    console.log(`[Express Shopify API] Successfully created fulfillment for Shopify order ${orderId}.`);
    ...
    console.error(`[Express Shopify API] Error in createShopifyFulfillment for order ${orderId}:`, error.message);
    throw error;
  }
}

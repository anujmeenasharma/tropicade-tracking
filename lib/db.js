...
export function getTrackings() {
    if (!fs.existsSync(dataFile)) {
        try {
            // Give preference to the root dataFile if copying to tmp
            if (isProduction && fs.existsSync(path.join(process.cwd(), 'lib', 'data.json'))) {
                fs.copyFileSync(path.join(process.cwd(), 'lib', 'data.json'), dataFile);
            } else {
                fs.writeFileSync(dataFile, JSON.stringify(defaultData, null, 2));
            }
        } catch (e) {
            console.error("Failed to initialize data.json", e);
        }
    }
    ...
    try {
        ...
        console.error("Error reading db. Fallback to cache/default:", error);
